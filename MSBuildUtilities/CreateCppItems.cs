...
public class CreateCppItems : Microsoft.Build.Utilities.Task
{
    ...
    public sealed override bool Execute()
    {
        ...
        ReadOnlySpan<ITaskItem> cppSource = CppSource;
        for (int i = 0; i < cppSource.Length; i++)
        {
            ref readonly ITaskItem item = ref cppSource[i];
            string itemsDir = item.ToString();
            bool modified = false;

            string previousDir = "";
            ReadOnlySpan<ITaskItem> configPerDirectory = ConfigPerDirectory;
            for (int j = 0; j < configPerDirectory.Length; j++)
            {
                ref readonly ITaskItem configPerDir = ref configPerDirectory[j];
        ...
        CppSource = newCppSource.Count != 0 ? [.. newCppSource] : [];
